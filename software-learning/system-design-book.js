(() => {
  ...
  function renderChapter() {
    ...
    document.title = `Chapter ${chapter.order}｜${section.title}`;
    ...
    document.querySelector('#bookSectionNav').innerHTML = chapter.sections.map(s => {
      ...
      return `<a class="${s.id === section.id ? 'active' : ''}" href="system-design-chapter.html?chapter=${chapter.id}&section=${s.id}"><span>${done ? '✓' : s.order}</span><div><b>${esc(s.title)}</b><small>${done ? '已完成' : '未完成'}</small></div></a>`;
    }).join('');
    ...
    document.querySelector('#bookSectionTitle').textContent = section.title;
    ...
    document.querySelector('#bookPageControls').innerHTML = `${prev ? `<a class="button secondary" href="?chapter=${chapter.id}&section=${section.id}&page=${prev.id}">← 上一頁</a>` : '<span></span>'}<div class="book-page-dots">${section.pages.map((p, i) => `<a class="${i === pageIdx ? 'active' : ''}" href="?chapter=${chapter.id}&section=${section.id}&page=${p.id}" aria-label="第 ${i + 1} 頁"></a>`).join('')}</div>${next ? `<a class="button" href="?chapter=${chapter.id}&section=${section.id}&page=${next.id}">下一頁 →</a>` : '<span></span>'}`;
  ...
  function renderSectionQuiz(chapter, section, root, map) {
    ...
    root.innerHTML = `<header><span>SECTION CHECK</span><h2>${esc(section.title)}｜小節練習</h2><p>${questions.length} 題。送出後會逐題診斷錯誤思路，並指出應回 Chapter ${chapter.order} 教材第幾頁複習。</p></header><div class="book-quiz-list">${questions.map((q, i) => renderQuestion(q, i)).join('')}</div><button class="button book-submit" type="button">送出小節練習</button><div class="book-quiz-result" hidden></div>`;
    ...
    root.querySelector('.book-submit').onclick = () => {
      ...
      result.hidden = false;
      result.innerHTML = `<div class="book-result-score">${score} 分</div><p>答對 ${correct}/${questions.length} · 本小節最高 ${saved.bestScore} 分</p><div class="result-actions">${nextSection ? `<a class="button" href="system-design-chapter.html?chapter=${chapter.id}&section=${nextSection.id}">進入下一小節：${nextSection.order}. ${esc(nextSection.title)} →</a>` : `<a class="button" href="system-design-exam.html?chapter=${chapter.id}">所有小節完成，進入 ${chapter.finalExam.length} 題 Chapter Exam →</a>`}<button class="button secondary" onclick="location.reload()">重新練習</button></div>`;
