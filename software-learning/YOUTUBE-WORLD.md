...
開啟 `system-design-simulator.html?chapter=sd-book-14`。原 12 月策略課程仍可從頁面連結進入（`&mode=lesson`）；其他章節與自由沙盒沿用原引擎。

## 操作

- 預設 100 人包含「我的角色」，持續觀看、搜尋、上傳與離開。可加到 500 人；超過 100 時畫布抽樣顯示，指標計算全部 session。
...
瀏覽器實測涵蓋桌面與窄視窗、弱網等待、背景持續運行、自動故障／恢復、按鈕關機／恢复、加機、加人、建立據點。正式部署尚未執行。
