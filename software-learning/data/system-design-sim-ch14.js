(() => {
  ...
  window.SYSTEM_DESIGN_SIM['sd-book-14'] = {
    ...
    abrSim: {
      ...
      desc: '進入模擬後，這部 1 小時影片會立刻開始加速播放；它被切成 720 個、每個 5 秒的片段。第一段已預先載入，所以前五秒正常播放；同時下載第二段。台灣 CDN 的 720p 片段能在緩衝用完前抵達，美國來源站的同一顆 720p 球因為位元組較大又跨海，會下載超過五秒，播放器先轉圈，再由 ABR 改抓更小、更快的低解析度片段。進入訊號不良區時，正在下載的片段也會立即受 0.4 Mbps 最後一哩限制，CDN 無法繞過。機器關閉時原球會在中途標紅並記錄失敗，保留進度與緩衝後重試；同區沒有健康服務時會改由美國等其他地區接手，畫面會標出實際服務路徑。',
      segments: 720,
      segmentSec: 5,
      simSecondMs: 50,
      maxBufferSec: 15,
    ...
    dragViewerSim: {
      homeRegionId: 'tw',
      start: { x: 190, y: 66 },
      zone: { x: 550, y: 260, width: 300, height: 80, label: '📶 訊號不良區 · 0.4 Mbps（可拖曳）' }
    },
    events: [
      {
        month: 2,
