...

{
 ...
 quiz:[
  MC("sd15-s01-q1","這一題的單一檔案大小上限是？","sd15-s01-p01","面試官明確給了 10 GB 的上限。","單檔大小上限為 10 GB",[["無任何檔案大小傳輸限制","題目有明確限制。"],["平均檔案大小為 500 KB","那是平均檔案大小的假設。"],["單一區塊大小為 4 MB","那是區塊的大小。"]]),
  MC("sd15-s01-q2","本章明確排除了哪一項功能？","sd15-s01-p02","多人同時編輯同一份文件超出設計範圍。","多人即時編輯與文件線上協作功能",[["跨多裝置的檔案自動同步更新功能","這是核心功能。"],["檔案歷史修訂記錄與版本復原功能","這是要做的功能。"],["多使用者之間共享檔案與存取權限","這是要做的功能。"]]),
  MC("sd15-s01-q3","排在第一位的非功能需求是？","sd15-s01-p03","對儲存系統來說，資料丟失是不可接受的。","資料不遺失的可靠性",[["跨裝置的即時同步速度","重要，但排在可靠性之後。"],["行動裝置的頻寬使用率","重要，但不是第一。"],["面對大量使用者的可擴展性","重要，但不是第一。"]]),
  MC("sd15-s01-q4","為什麼「頻寬使用率」會被列進非功能需求？","sd15-s01-p03","手機常使用流量有限的方案，浪費頻寬使用者一定不滿意。","行動方案流量有限，過度消耗頻寬極度影響體驗",[["因為雲端儲存伺服器向電信業者租用頻寬成本極高","那是成本問題，這裡講的是使用者體驗。"],["因為國際雲端儲存安全法規強制規範傳輸流量上限","題目沒有這個前提。"],["因為啟用端到端對稱加密技術必然導致傳輸負載暴增","加密與頻寬無直接關係。"]])
 ]
},

{
 ...
 quiz:[
  MC("sd15-s02-q1","500 PB 這個總容量是怎麼算的？","sd15-s02-p02","配置給每一個註冊帳號的空間都要留著。","5,000 萬註冊使用者 × 每人 10 GB 空間",[["1,000 萬日活躍使用者 × 10 GB 儲存配額","空間是配置給註冊帳號，不是只給今天活躍的人。"],["每日平均新增上傳資料量 × 365 天年增累積","那是年增量不是總配置。"],["單次上傳檔案 500 KB × 全年累積上傳總次數","那是每日新增資料量。"]]),
  MC("sd15-s02-q2","上傳 API 的峰值 QPS 約為？","sd15-s02-p03","平均 240，峰值取 2 倍。","尖峰峰值約 480 QPS",[["平均估算約 240 QPS","那是平均值。"],["高估預估達 2,400 QPS","高估了一個數量級。"],["低估預估僅 48 QPS","低估了。"]]),
  MC("sd15-s02-q3","240 QPS 這個數字告訴我們什麼？","sd15-s02-p03","請求數本身很小，難點在位元組量與不能遺失。","本題的核心瓶頸在位元組儲存而非 QPS 請求量",[["前端系統必須立即部署數千台 API 伺服器集群","240 QPS 不需要。"],["後台資料庫必須在一開始就進行跨分區水平分片","不是由這個數字決定的。"],["所有檔案的二進位上傳傳輸必須全面強制走 CDN","上傳流量不走 CDN。"]]),
  MC("sd15-s02-q4","讀寫比率 1 : 1 的意義是？","sd15-s02-p01","下載與上傳的量級相當，兩條路徑都要認真設計。","下載與上傳量相當，讀取路徑同等重要需認真設計",[["讀取流量遠大於寫入流量，架構設計只需專注快取","比率是 1:1。"],["寫入資料量遠高於讀取需求，讀取延遲可以完全忽略","比率是 1:1。"],["代表雲端快取層在讀寫轉換過程中的命中率剛好五成","與快取無關。"]])
 ]
},

{
 ...
 quiz:[
  MC("sd15-s03-q1","檔案在單一伺服器版本裡是怎麼被唯一識別的？","sd15-s03-p02","名稱空間對應使用者，相對路徑對應檔案位置。","使用者名稱空間加上檔案相對路徑",[["僅依賴檔案本身的原始自訂檔名","不同使用者可能有同名檔案。"],["依據二進位資料計算之檔案雜湊值","那是區塊層級的識別。"],["伺服器接收到請求時的上傳時間戳","不保證唯一。"]]),
  MC("sd15-s03-q2","什麼情況下該用斷點續傳而不是簡單上傳？","sd15-s03-p03","檔案比較大、而且網路中斷的可能性很高。","大檔案傳輸搭配不穩定的行動網路",[["系統中任何大小的檔案上傳皆應採用","小檔案用簡單上傳就好。"],["僅適用於純文字檔案或原始程式碼檔","與檔案類型無關。"],["僅在使用者主動進行跨帳號檔案分享時","與分享無關。"]]),
  MC("sd15-s03-q3","斷點續傳的第一個步驟是？","sd15-s03-p03","要先拿到一個可以續傳的網址。","向伺服器發送請求，取得可續傳專用網址",[["直接在傳輸層無條件開始推送檔案二進位位元組","沒有續傳網址就無法恢復。"],["由客戶端預先完整計算整份檔案的全局雜湊碼","不是第一步。"],["向發布訂閱服務發出推播以通知所有關聯裝置","不是上傳流程的第一步。"]]),
  MC("sd15-s03-q4","為什麼要先做單一伺服器版本？","sd15-s03-p01","每個新元件都應該是被具體問題逼出來的。","清楚看見系統瓶頸，讓每次架構演進都有明確依據",[["因為單一伺服器在正式生產環境就能承載全域儲存","很快就會空間不足。"],["因為面試評分標準硬性規定必須先從單機程式碼起步","要能自己說出理由。"],["純粹為了在專案初期盡可能節省伺服器硬體採購成本","理由不是成本。"]])
 ]
},

{
 ...
 quiz:[
  MC("sd15-s04-q1","分片解決了什麼問題，又沒解決什麼問題？","sd15-s04-p02","分片讓你放得下，但沒有讓你放得住。","分片解決了儲存容量問題，但未解決資料耐久性",[["分片機制同時徹底解決了容量上限與硬體損毀遺失","單一儲存伺服器壞掉仍會丟資料。"],["分片僅能提升資料耐久性，對擴展儲存容量毫無幫助","剛好相反。"],["分片在架構上既不能擴充容量也無法提升資料可靠度","它確實解決了容量。"]]),
  MC("sd15-s04-q2","為什麼要把檔案複製到兩個不同的地理區域？","sd15-s04-p02","連整個區域出事都還有資料。","提供跨區域冗餘備援，防止硬體損毀並確保可用性",[["純粹為了藉由縮短實體網路距離以加快上傳傳輸速度","跨區複製不會讓上傳更快。"],["透過資料壓縮降低多個雲端服務商之間的儲存租用費","複製會增加成本。"],["為了符合作業系統對單一資料中心最大檔案大小限制","與大小無關。"]]),
  MC("sd15-s04-q3","把 metadata 資料庫移出伺服器的主要理由？","sd15-s04-p03","資料庫留在單機上就是一個單點故障。","消除單點故障，並讓資料庫能獨立配置分片與複寫",[["主要為了大幅提升關聯式資料庫對外查詢的極限延遲","主要理由是可用性。"],["因為中繼資料檔案過大，單一伺服器記憶體完全放不下","不是主因。"],["為了在作業系統檔案層面全面原生支援二進位透明加密","與加密無關。"]]),
  MC("sd15-s04-q4","儲存桶（bucket）相當於什麼？","sd15-s04-p02","S3 用桶來組織物件。","類似檔案系統中用來組織檔案的根資料夾",[["機房內部負責處理儲存運算的一台實體伺服器","桶是邏輯容器不是機器。"],["檔案切分過程中固定大小為 4MB 的資料區塊","區塊是檔案切出來的片段。"],["在身分認證服務中用來登記個人檔案的使用者帳號","不是。"]])
 ]
},

{
 ...
 quiz:[
  MC("sd15-s05-q1","兩人同時修改同一個檔案時，本章採用的策略是？","sd15-s05-p01","先處理到的為準。","先處理完畢的版本為準，較晚提交者收到衝突通知",[["後到達的修改版本無條件覆蓋先處理完成的歷史版本","會讓先提交的人資料消失。"],["為了維持資料完整性，系統將兩份發生衝突的檔案皆丟棄","違反資料不可丟失。"],["由伺服器在兩份衝突版本中隨機挑選一份作為正式檔案","不可預測。"]]),
  MC("sd15-s05-q2","系統怎麼處理那個發生衝突的使用者？","sd15-s05-p02","兩個版本都保留，讓使用者自己決定。","同時呈現本機複本與伺服器最新版，由使用者決定",[["直接丟棄衝突使用者的所有本機修改以維持一致性","資料丟失不可接受。"],["系統在背景透過演算法自動合併，完全不通知使用者","可能合併錯誤且他不知情。"],["在伺服器端全面鎖定檔案並禁止該使用者再次編輯操作","題目沒有採用鎖定策略。"]]),
  MC("sd15-s05-q3","為什麼不乾脆自動合併就好？","sd15-s05-p02","系統無法知道哪一份才是使用者想要的。","交由使用者抉擇較安全，避免自動猜測導致資料誤損",[["因為電腦科學領域中完全不存在任何自動文本合併演算法","存在，但不保證正確。"],["因為執行文字差異比較與合併的演算法執行效能過於緩慢","主要理由不是效能。"],["因為需要合併的檔案二進位體積過於龐大而無法載入記憶體","與大小無關。"]]),
  MC("sd15-s05-q4","多人即時協作編輯在這一題的定位是？","sd15-s05-p03","第一步就明確排除了。","超出本章設計範圍，需依賴協作專用演算法",[["屬於本章核心功能，需由區塊伺服器完整實現","被明確排除。"],["只要透過衝突副本機制即可解決多人同時輸入","即時協作需要不同的方法。"],["只要在中繼資料庫加上檔案排他鎖即可達成協作","不是本章的解法。"]])
 ]
},

{
 ...
 quiz:[
  MC("sd15-s06-q1","區塊伺服器的職責是？","sd15-s06-p02","把檔案切成區塊再上傳到雲端儲存。","將檔案切分成區塊、壓縮加密並上傳至雲端儲存",[["負責驗證客戶端登入權限與管理帳號個人檔案資訊","那是 API 伺服器的工作。"],["在檔案發生變動時主動推播通知至其他客戶端設備","那是通知服務。"],["在分散式關聯資料庫中記錄檔案版本的中繼資料","那是 metadata 資料庫。"]]),
  MC("sd15-s06-q2","API 伺服器負責什麼？","sd15-s06-p03","它負責上傳流程以外幾乎所有的其他工作。","身分驗證、個人檔案管理與中繼資料更新",[["搬運龐大檔案之二進位位元組傳輸管線","那走區塊伺服器。"],["儲存使用者檔案之實體底層區塊位元組","檔案在雲端儲存系統。"],["在伺服器端將檔案切分成固定大小區塊","那是區塊伺服器。"]]),
  MC("sd15-s06-q3","本章參考的區塊最大尺寸是？","sd15-s06-p02","用 Dropbox 的設定作為參考。","參考 Dropbox 設計的 4 MB",[["單一檔案傳輸上限 10 GB","那是單檔上限。"],["平均檔案大小假設 500 KB","那是平均檔案大小。"],["作業系統標準頁面 256 KB","不是本章採用的數字。"]]),
  MC("sd15-s06-q4","離線備份佇列存在的理由？","sd15-s06-p03","客戶端離線時變動不能就這樣消失。","暫存離線期間的變更通知，待裝置重新連線後同步",[["大幅提升客戶端直接上傳大型二進位檔案的速度","與上傳速度無關。"],["在伺服器內部持久化儲存檔案實際的二進位位元組","它存的是變動資訊。"],["徹底取代 metadata 資料庫以避免任何磁碟寫入開銷","不是。"]])
 ]
}
